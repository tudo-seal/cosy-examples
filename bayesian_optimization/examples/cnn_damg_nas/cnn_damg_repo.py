...
class CNNrepository:
    ...
    def __init__(self,
        ...
        # back to the original dense range - this keeps the DNN behaviour unchanged.
    ...
    # dataclass/interpretation layer; the type system (Para/beside/before/swap) never sees it.
    @dataclass(frozen=True)
    class Conv2d:
        in_channels: int
        out_channels: int
    ...
    class Label(Group):
        ...
        def iter_conv2d(self):
            ...
            # sizes) - this is what keeps the wiring layer (Para/beside/swap) sparse, see __init__.
                                ...
                                for padding in self.padding_values:
                                    ...
                                    if i in self.linear_feature_dimension_set and o in self.linear_feature_dimension_set:
                                        yield CNNrepository.Conv2d(
                                            ...
                                            kernel_size=(kh, kw), stride=stride, padding=padding, bias=True)
                                        yield CNNrepository.Conv2d(
                                            in_channels=in_c, out_channels=out_c,
                                            input_size=(in_h, in_w), output_size=(out_h, out_w),
        ...
        def __iter__(self):
            if self._iter_cache is None:
                self._iter_cache = tuple(
                     ...
                     *self.iter_conv2d(), *self.iter_maxpool2d()]
                )
    ...
    def specification(self):
        ...
        # multiplicity, not a feature-dimension value. In the plain DNN case max(self.dimensions) stayed
        # small, so reusing it here was harmless; with sparse conv features self.dimensions can contain
        # large flattened values (e.g. 3072), which would make max_length itself explode combinatorially.
        ...
        return {
            ...
            # Loss slot in a target can safely stay a wildcard (Literal(None)) - there is only one
            # valid combinator left for synthesis to pick.
